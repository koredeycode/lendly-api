CREATE TYPE "public"."booking_status" AS ENUM('pending', 'accepted', 'picked_up', 'returned', 'completed', 'cancelled', 'overdue');--> statement-breakpoint
...
CREATE TYPE "public"."wallet_transaction_type" AS ENUM('deposit', 'rental_payment', 'tip_payment', 'rental_receive', 'tip_receive', 'refund', 'withdrawal');--> statement-breakpoint
CREATE TABLE "users" (
...
CREATE INDEX "idx_wallet_tx_booking" ON "wallet_transactions" USING btree ("booking_id");--> statement-breakpoint
...
CREATE INDEX "idx_chat_booking" ON "chat_messages" USING btree ("booking_id","created_at" DESC NULLS LAST);